  ...
  class Association # :nodoc:
    ...
    def self.add_destroy_callbacks(model, reflection)
      ...
      if reflection.deprecated?
        # If :dependent is set, destroying the parent record has some sort of
        # side-effect, could restrict, could nullify FKs, etc. We want to notify
        # about destroys, because the side-effect would not longer happen and
        # code could be relying on it.
        model.before_destroy { ActiveRecord::Associations::Deprecation.warn(reflection) }
      end
    end
    ...
    def self.guard_deprecated_access(reflection)
      ActiveRecord::Associations::Deprecation.guard_deprecated_access(reflection)
    end
